    ...
    <div class="gallery">
      ...
      gallery.map((f) => {
        ...
        if (item.isPlayable) {
          ...
          return `<div class="gallery-item" data-src="${item.src}" data-type="${item.type}" data-name="${item.name.replace(/"/g, '"')}" style="cursor:pointer;"><div class="media-preview"><span class="icon-placeholder">📄</span></div><div class="card-info"><div class="card-filename" title="${item.name.replace(/"/g, '"')}">${item.name}</div><div class="card-row-details"><span class="badge-type badge-doc">Dokumen</span><span>${hoverCardHTML}</span></div><div class="card-row-details" style="margin-top:2px; color:var(--text-muted);"><span>💾 ${item.size}</span><span>🕒 ${item.time}</span></div><div style="margin-top:6px;"><button class="btn-download-single" data-src="${item.src}" style="width:100%; padding:4px 8px; background:var(--bg-tertiary); border:1px solid var(--border); border-radius:4px; color:var(--accent-blue); font-size:0.7rem; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background='var(--accent-blue)'; this.style.color='#fff';" onmouseout="this.style.background='var(--bg-tertiary)'; this.style.color='var(--accent-blue)';">💾 Download</button></div></div></div>`;
        }
      }).join('')}
    </div>
