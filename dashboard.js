...
// ==========================================
// API DOWNLOAD SINGLE MEDIA FILE
// ==========================================
app.get("/api/download-media", async (req, res) => {
  const filePath = req.query.file;
  if (!filePath) return res.status(400).json({ error: "Parameter file wajib diisi" });
  
  // Decode path dan validasi keamanan
...
// ==========================================
// API DOWNLOAD ZIP PER GRUP + RENTANG TANGGAL
// ==========================================
app.get("/api/download-zip", async (req, res) => {
  ...
  if (!group || !startDate || !endDate) {
    return res.status(400).json({ error: "Grup, tanggal mulai, dan tanggal selesai wajib diisi" });
  }
  
  const groupDir = path.join(MEDIA_DIR, group);
  if (!fs.existsSync(groupDir)) {
    return res.status(404).json({ error: "Grup tidak ditemukan" });
  }
  
  // Format tanggal untuk nama folder: DDMMYYYY
  const formatDateForFolder = (dateStr) => {
  ...
  <main class="main-content">
    ...
    <div class="gallery">
      ...
      gallery.map((f) => {
        ...
        const _q = String.fromCharCode(38) + 'quot;';
        const safeName = item.name.split('"').join(_q);
        if (item.isPlayable) {
          return `<div class="gallery-item" data-src="${item.src}" data-type="${item.type}" data-name="${safeName}" style="cursor:pointer;"><div class="media-preview">${item.type === 'image' ? '<img src="' + item.src + '" loading="lazy">' : '<span class="icon-placeholder">' + (item.type === 'video' ? '🎥' : '🎵') + '</span>'}</div><div class="card-info"><div class="card-filename" title="${safeName}">${item.name}</div><div class="card-row-details"><span class="badge-type badge-${item.type}">${item.type}</span><span>${hoverCardHTML}</span></div><div class="card-row-details" style="margin-top:2px; color:var(--text-muted);"><span>💾 ${item.size}</span><span>🕒 ${item.time}</span></div><div style="margin-top:6px;"><button class="btn-download-single" data-src="${item.src}" style="width:100%; padding:4px 8px; background:var(--bg-tertiary); border:1px solid var(--border); border-radius:4px; color:var(--accent-blue); font-size:0.7rem; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background='var(--accent-blue)'; this.style.color='#fff';" onmouseout="this.style.background='var(--bg-tertiary)'; this.style.color='var(--accent-blue)';">💾 Download</button></div></div></div>`;
        } else {
          return `<div class="gallery-item" data-src="${item.src}" data-type="${item.type}" data-name="${safeName}" style="cursor:pointer;"><div class="media-preview"><span class="icon-placeholder">📄</span></div><div class="card-info"><div class="card-filename" title="${safeName}">${item.name}</div><div class="card-row-details"><span class="badge-type badge-doc">Dokumen</span><span>${hoverCardHTML}</span></div><div class="card-row-details" style="margin-top:2px; color:var(--text-muted);"><span>💾 ${item.size}</span><span>🕒 ${item.time}</span></div><div style="margin-top:6px;"><button class="btn-download-single" data-src="${item.src}" style="width:100%; padding:4px 8px; background:var(--bg-tertiary); border:1px solid var(--border); border-radius:4px; color:var(--accent-blue); font-size:0.7rem; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background='var(--accent-blue)'; this.style.color='#fff';" onmouseout="this.style.background='var(--bg-tertiary)'; this.style.color='var(--accent-blue)';">💾 Download</button></div></div></div>`;
        }
      }).join('')}
    </div>
    ...
    if (result.data && result.data.length > 0) {
      ...
      result.data.forEach(item => {
        ...
        if (item.isPlayable) {
          ...
          html = '<div class="gallery-item" data-src="' + item.src + '" data-type="' + item.type + '" data-name="' + item.name + '" style="cursor:pointer;"><div class="media-preview"><span class="icon-placeholder">📄</span></div><div class="card-info"><div class="card-filename" title="' + item.name + '">' + item.name + '</div><div class="card-row-details"><span class="badge-type badge-doc">Dokumen</span><span>' + hoverCardHTML + '</span></div><div class="card-row-details" style="margin-top:2px; color:var(--text-muted);"><span>💾 ' + item.size + '</span><span>🕒 ' + item.time + '</span></div><div style="margin-top:6px;"><button class="btn-download-single" data-src="' + item.src + '" style="width:100%; padding:4px 8px; background:var(--bg-tertiary); border:1px solid var(--border); border-radius:4px; color:var(--accent-blue); font-size:0.7rem; cursor:pointer; transition:all 0.2s;" onmouseover="this.style.background=\'var(--accent-blue)\'; this.style.color=\'#fff\';" onmouseout="this.style.background=\'var(--bg-tertiary)\'; this.style.color=\'var(--accent-blue)\';">💾 Download</button></div></div></div>';
        }
